...
| `O_j ~ Poisson(S)`                              | Asymptotic Poisson approximation of column load                           | Monte-Carlo evaluation                      |
| `O_{ij} = randomized_top_k(tilde O_{i*}, k)`    | Row fan-out truncation with seed-deterministic randomized selection       | `phase_router.py` implementation            |
...
This approximation holds for large `N` and moderate degrees; **truncation by randomized top-k introduces slight negative correlations**.

---

## 6. Fan-Out Truncation with Seed-Deterministic Randomized Top-k Selection

In the implementation, each source row emits at most `k` matches. Let `tilde O_{ij}` denote the uncapped intersections from Section 4. If more than `k` overlaps exist for row `i`, **up to `k` are selected via a seed-deterministic random choice**:

```
O_{ij} = randomized_top_k(tilde O_{i*}, k, seed)
```

This guarantees:

- **Hard fan-out bound**: sum*j O*{ij} ≤ k for every row
- **Reduced column skew**: avoids systematic overload of early columns
- **Deterministic reproducibility**: fixed seed produces identical routing

Columns with fewer than k overlaps retain all matches. This randomized selection preserves the Chung–Lu first-order statistics in expectation, while improving practical load balance for small k and uneven degree distributions.
...
4. **Seed-deterministic top-k selection** enforces row capacity constraints.
...
Collecting `M` independent seeds:
...
Seed-deterministic top-k ensures reproducibility across repeated experiments while allowing fair column load distribution.
...
- finite `k`,
- row capacity constraints (top-k),
