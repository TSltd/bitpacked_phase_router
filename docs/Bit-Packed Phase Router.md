...
Let (S, T \in {0,1}^{N \times N}) denote source and target matrices with row sums (s_i) and column sums (t_j). We aim to construct a routing matrix (O) such that:
...
[
\Pr(O_{ij}=1) \approx \frac{s_i t_j}{N}
]
